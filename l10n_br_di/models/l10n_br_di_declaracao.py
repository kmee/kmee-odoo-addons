...
class L10nBrDiDeclaracao(models.Model):
    ...
    def _currency_search(self, siscomex_code):
        return self.env["res.currency"].search(
            [("siscomex_code", "=", siscomex_code)], limit=1
        )
    ...
    def _importa_declaracao(self, declaracoes):
        ...
        insurance_currency_id = self.env["res.currency"].search(
            [("siscomex_code", "=", di.seguro_moeda_negociada_codigo)],
            limit=1,
        )
        freight_currency_id = self.env["res.currency"].search(
            [("siscomex_code", "=", di.frete_moeda_negociada_codigo)],
            limit=1,
        )
        ...
        vals = {
            ...
            "via_transporte_pais_transportador_codigo":
                di.via_transporte_pais_transportador_codigo,
        }
        return vals
