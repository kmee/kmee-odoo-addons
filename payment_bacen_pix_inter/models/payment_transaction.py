...
class PaymentTransaction(models.Model):
    ...
    # Campos PIX Inter
    pix_txid = fields.Char("TXID Pix Inter", readonly=True)
    ...
    ], string="Status PIX", readonly=True)
    ...
    def _generate_cert_files(self, provider_id):
        ...
        cert = base64.b64decode(provider_id.inter_pix_certificate)
        key = (
            base64.b64decode(provider_id.inter_pix_private_key)
            if provider_id.inter_pix_private_key
            else None
        )
    ...
    def _generate_header(self, token, provider_id):
        ...
        return {
            ...
            "x-conta-corrente": provider_id.bacen_pix_key,
        }
    ...
    def _get_pix_token(self, provider_id, scope):
        ...
        try:
            ...
            if not token:
                raise UserError(
                    _("Token PIX não encontrado na resposta da API")
                )
    ...
    def generate_pix(self):
        ...
        try:
            ...
            self.write({
                ...
                'pix_location': response.get("location"),
                'pix_qrcode': response.get("pixCopiaECola"),
                ...
                if self.pix_txid == txid:
                    ...
                    if current_status:
                        ...
                        if current_status == 'CONCLUIDA':
                            self._set_done()
                        elif current_status in ['REMOVIDA_PELO_USUARIO_RECEBEDOR', 'REMOVIDA_PELO_PSP']:
                            self._set_canceled()
    ...
    def _cron_update_pix_status(self):
        ...
        for transaction in pending_pix_transactions:
            try:
                transaction.get_pix_inter(transaction.provider_id, transaction.pix_txid)
            except Exception as e:
                _logger.error(
    ...
    def action_update_pix_status(self):
        ...
        if self.provider_code != "inter_pix":
            raise ValidationError(_("Esta ação é apenas para transações PIX do Banco Inter"))
            
        return self.get_pix_inter(self.provider_id, self.pix_txid)
    ...
    def action_create_pix(self):
        ...
        if self.provider_code != 'inter_pix':
            raise ValidationError(_("Este método é apenas para transações PIX do Banco Inter"))
    ...
    def create(self, vals):
        ...
        if (transaction.provider_code == 'inter_pix' and 
            transaction.operation == 'online_redirect' and
